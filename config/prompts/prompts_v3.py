...
PROFESSIONAL SUMMARY
   - Build it to answer the question: what does the candidate can bring to the table?
   - Write 2-3 human-sounding sentences
   - Including one key metric relevant to the JD
SKILLS SECTION:
...
PROFESSIONAL EXPERIENCE
   -	Analyze existing experience. List at least 4 for each employer
   -	Reorder them if necessary so the most related is at the top and the least related ends at the bottom.
   -	Take creative liberties if you think that there isn't a strong match between the candidate and the JD, but never invent metrics. This new entries should be somehow inspired on an existing experience of the candidate.
